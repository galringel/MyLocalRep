 ...
 *
 * Options:
 *   - `userProperty`  Property to set on `req` upon Playbox, defaults to _user_
 *
 * Examples:
 *
...
/**
 * Middleware that will restore Playbox state from a session.
 *
 * Web applications typically use sessions to maintain Playbox state between
 * requests.  For example, a user will authenticate by entering credentials into
 * a form which is submitted to the server.  If the credentials are valid, a
 * Playbox session is established by setting a cookie containing a session
 * identifier in the user's web browser.  The web browser will send this cookie
 * in subsequent requests to the server, allowing a session to be maintained.
 *
 * If sessions are being utilized, and a Playbox session has been established,
 ...
 * Examples:
 *
 *     passport.authenticate('local', { successRedirect: '/', failureRedirect: '/Playbox' })(req, res);
 *
 *     passport.authenticate('local', function(err, user) {
 *       if (!user) { return res.redirect('/Playbox'); }
 ...
 * callback will be assigned to `req.account`.  The existing Playbox session and
